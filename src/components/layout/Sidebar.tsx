...
export function Sidebar({ open, onClose }: SidebarProps) {
  ...
  const p = {
    thisMonth:       usePagePermission('this-month'),
    stats:           usePagePermission('stats'),
    clients:         usePagePermission('clients'),
    projects:        usePagePermission('projects'),
    internal:        usePagePermission('internal'),
    maintenances:    usePagePermission('maintenances'),
    contracts:       usePagePermission('contracts'),
    contractors:     usePagePermission('contractors'),
    sales:           usePagePermission('sales'),
    infrastructure:  usePagePermission('infrastructure'),
    domains:         usePagePermission('domains'),
    planning:        usePagePermission('planning'),
    stack:           usePagePermission('stack'),
    forecast:        usePagePermission('forecast'),
    resourcePlanning: usePagePermission('resource-planning'),
  ...
  return (
    <aside className={`sidebar${open ? ' sidebar-open' : ''}`}>
      ...
      <nav className="sidebar-nav" onClick={onClose}>
        ...
        {p.pixel.canView && <NavLink to="/pixel" className={nav}><IconSparkle /> Pixel AI</NavLink>}

        <div className="sidebar-divider" />

        <div className="sidebar-group-label">Invoicing</div>
        {p.thisMonth.canView && <NavLink to="/this-month"   className={nav}><IconInvoice /> This Month</NavLink>}
        {p.stats.canView     && <NavLink to="/stats"        className={nav}><IconBarChart /> Statistics</NavLink>}

        <div className="sidebar-divider" />

        <div className="sidebar-group-label">Clients &amp; Projects</div>
        {p.clients.canView      && <NavLink to="/clients"       className={nav}><IconUsers />    Clients</NavLink>}
        {p.projects.canView     && <NavLink to="/projects"      className={nav}><IconBriefcase /> Projects</NavLink>}
        {p.internal.canView     && <NavLink to="/internal"      className={nav}><IconGrid /> Internal</NavLink>}
        {p.maintenances.canView && <NavLink to="/maintenances"  className={nav}><IconTool /> Maintenances</NavLink>}
        {p.contracts.canView    && <NavLink to="/contracts"     className={nav}>
        ...
        {p.contractors.canView  && <NavLink to="/contractors"   className={nav}>
        ...
        {p.sales.canView        && <NavLink to="/sales"         className={nav}><IconTarget /> Sales</NavLink>}

        <div className="sidebar-divider" />

        <div className="sidebar-group-label">Domains &amp; Hosting</div>
        {p.infrastructure.canView && <NavLink to="/infrastructure" className={nav}><IconServer /> Hosting</NavLink>}
        {p.domains.canView        && <NavLink to="/domains"        className={nav}><IconGlobe /> Domains</NavLink>}

        <div className="sidebar-divider" />

        <div className="sidebar-group-label">Finance</div>
        {p.planning.canView && <NavLink to="/planning" className={nav}><IconCalendar /> Invoice Plan</NavLink>}
        {p.stack.canView    && <NavLink to="/stack" className={nav}>
        ...
        {p.forecast.canView && <NavLink to="/forecast" className={nav}>
        ...
        {p.resourcePlanning.canView && <NavLink to="/resource-planning" className={nav}><IconGrid /> Allocation</NavLink>}
        {p.reports.canView          && <NavLink to="/reports" className={nav}><IconBarChart /> Reports</NavLink>}
        {p.resourceYearly.canView   && <NavLink to="/resource-yearly" className={nav}><IconCalendar /> Yearly Plan</NavLink>}
        ...
        {p.tools.canView      && <NavLink to="/tools" className={nav}><IconTool /> Tools</NavLink>}
